...
#pragma warning disable 0414
        ...
        /// <summary>
        /// The step capsule of this motor
        /// </summary>
        [ReadOnly]
        public CapsuleCollider StepCapsule;
        /// <summary>
        /// The character capsule of this motor
        /// </summary>
        ...
        /// Vector3 from the character transform position to the capsule bottom hemi center
        /// </summary>
        public Vector3 CharacterTransformToCharacterCapsuleBottomHemi { get; private set; }
        /// <summary>
...
#pragma warning restore 0414 
        ...
        public void ValidateData()
        {
            ...
            StepCapsule.isTrigger = false;
            StepCapsule.direction = 1;
            StepCapsule.sharedMaterial = CapsulePhysicsMaterial;
...
#if UNITY_EDITOR
            StepCapsule.hideFlags = HideFlags.NotEditable;
        ...
        public void SetCapsuleDimensions(float radius, float height, float yOffset)
        {
            ...
            StepCapsule.radius = CapsuleRadius;
            StepCapsule.height = Mathf.Clamp(CapsuleHeight, CapsuleRadius * 2f, CapsuleHeight);
            StepCapsule.center = new Vector3(0f, CapsuleYOffset, 0f);

            CharacterTransformToCapsuleCenter = StepCapsule.center;
            CharacterTransformToCapsuleBottom = StepCapsule.center + (-_cachedWorldUp * (StepCapsule.height * 0.5f));
            CharacterTransformToCapsuleTop = StepCapsule.center + (_cachedWorldUp * (StepCapsule.height * 0.5f));
            CharacterTransformToCapsuleTopHemi = StepCapsule.center + (_cachedWorldUp * (StepCapsule.height * 0.5f)) + (-_cachedWorldUp * StepCapsule.radius);
            ...
            CharacterTransformToCharacterCapsuleBottomHemi = CharacterCapsule.center + (-_cachedWorldUp * (CharacterCapsule.height * 0.5f)) + (_cachedWorldUp * CharacterCapsule.radius);
        }
        ...
        public void UpdatePhase2(float deltaTime)
        {
            ...
            /*if (_solveMovementCollisions && InteractiveRigidbodyHandling)
            {
                if (InteractiveRigidbodyHandling)
                {
                    ...
                    if (AttachedRigidbody)
                    {
                        float upwardsOffset = StepCapsule.radius;
        ...
        public int CharacterOverlap(Vector3 atPosition, Quaternion atRotation, Collider[] overlappedColliders, LayerMask layers, QueryTriggerInteraction triggerInteraction, float radiusInflate = 0f)
        {
            ...
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                if (overlappedColliders[i] == StepCapsule || overlappedColliders[i] == CharacterCapsule)
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        public int CharacterCapsuleCollisionsSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, float radiusInflate = 0f)
        {
            direction.Normalize();
            ...
            int nbHits = 0;
            int nbUnfilteredHits = Physics.CapsuleCastNonAlloc(
                    position + (rotation * CharacterTransformToCharacterCapsuleBottomHemi) - (direction * SweepProbingBackstepDistance),
                    position + (rotation * CharacterTransformToCapsuleTopHemi) - (direction * SweepProbingBackstepDistance),
                    CharacterCapsule.radius + radiusInflate,
                    direction,
                    hits,
                    distance + SweepProbingBackstepDistance,
                    CollidableLayers,
        ...
        public int CharacterSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, LayerMask layers, QueryTriggerInteraction triggerInteraction, float radiusInflate = 0f)
        {
            ...
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                ...
                if (hits[i].distance <= 0f || hits[i].collider == StepCapsule || hits[i].collider == CharacterCapsule)
                {
                    nbHits--;
                    if (i < nbHits)
                    {
