...
#pragma warning disable 0414
        ...
        /// <summary>
        /// The capsule collider, used for collisions, of this motor
        /// </summary>
        public CapsuleCollider Capsule;
        /// <summary>
        /// The capsule collider, used for ground handling, of this motor
        /// </summary>
        public CapsuleCollider StepCapsule;
        /// <summary>
        ...
        /// <summary>
        /// Vector3 from the character transform position to the STEP capsule bottom hemi center
        /// </summary>
        public Vector3 CharacterTransformToStepCapsuleBottomHemi { get; private set; }
        /// <summary>
        ...
        private List<Collider> _validStepColliders = new List<Collider>();
...
#pragma warning restore 0414 
        ...
        private void HandleRemoveComponent()
        {
            ...
            CapsuleCollider tmpCapsule = Capsule;
        ...
        public void ValidateData()
        {
            ...
            MinRequiredStepDepth = Mathf.Clamp(MinRequiredStepDepth, 0f, StepCapsule.radius);

            MaxStableDistanceFromLedge = Mathf.Clamp(MaxStableDistanceFromLedge, 0f, StepCapsule.radius);
        ...
        public void SetCapsuleDimensions(float radius, float height, float yOffset)
        {
            ...
            //Must take into account both possible uses
            CharacterTransformToCapsuleBottomHemi = Capsule.center + (-_cachedWorldUp * (Capsule.height * 0.5f)) + (_cachedWorldUp * Capsule.radius);
            CharacterTransformToStepCapsuleBottomHemi = StepCapsule.center + (-_cachedWorldUp * (StepCapsule.height * 0.5f)) + (_cachedWorldUp * StepCapsule.radius);
        }
        ...
        public void UpdatePhase1(float deltaTime)
        {
            ...
            if (_solveMovementCollisions)
            {
                ...
                {
                    int nbOverlaps = StepCapsuleCollisionsOverlap(TransientPosition, TransientRotation, _internalProbedColliders);
                    ...
                    if (nbOverlaps > 0)
                    {
                        ...
                        for (int i = 0; i < nbOverlaps; i++)
                        {
                            ...
                            {
                                bool isStepCollider = false;

                                Debug.Log("2) performing loop. Count = " + _validStepColliders.Count);
                                for (int j = 0; j < _validStepColliders.Count; j++)
                                {
                                    if (_internalProbedColliders[i] == _validStepColliders[j])
                                    {
                                        isStepCollider = true;
                                        break;
                                    }
                                ...
                                if(!isStepCollider)
                                {
                                    // Process overlap from a non step collider
                                ...
                                {
                                    // Process overlap from a step collider
                                    Transform overlappedTransform = _internalProbedColliders[i].GetComponent<Transform>();
                                    if (Physics.ComputePenetration(
                                            StepCapsule,
                                            TransientPosition,
            ...
            if (_solveGrounding)
            {
                ...
                else
                {
                    ...
                    if (!LastGroundingStatus.SnappingPrevented && (LastGroundingStatus.IsStableOnGround || LastMovementIterationFoundAnyGround))
                    {
                        ...
                        {
                            selectedGroundProbingDistance = Mathf.Max(StepCapsule.radius, MaxStepHeight);
                        }
                        else
                        {
                            selectedGroundProbingDistance = StepCapsule.radius;
                        }
        ...
        private bool InternalCharacterMove(Vector3 movement, float deltaTime, out float resultingMovementMagnitude, out Vector3 resultingMovementDirection)
        {
            ...
            {
                if (StepCapsuleCollisionsSweep(                                                                       ///////////When this is StepCapsuleCollisionsSweep() everything works perfectly. May have to duplicate and have both
                    ...
                    > 0)
                {
                    ...
                    if (_solveGrounding && StepHandling != StepHandlingMethod.None && moveHitStabilityReport.ValidStepDetected)
                    {
                        ...
                        if (obstructionCorrelation <= CorrelationForVerticalObstruction)
                        {
                            ...
                            int nbStepHits = StepCapsuleCollisionsSweep(
        ...
        /// <summary>
        /// Determines if the input collider is valid for collision processing with the Character (Capsule and StepCapsule)
        /// </summary>
        /// <returns> Returns true if the collider is valid </returns>
        private bool CheckIfColliderValidForCollisionsWithCharacter(Collider coll)
        {
            // Ignore self
            if (coll == null ||
                coll == Capsule ||
        ...
        private void DetectSteps(Vector3 characterPosition, Quaternion characterRotation, Vector3 hitPoint, Vector3 innerHitDirection, ref HitStabilityReport stabilityReport)
        {
            ...
            stepCheckStartPos = characterPosition + (characterUp * MaxStepHeight) + (-innerHitDirection * StepCapsule.radius);
            nbStepHits = StepCapsuleCollisionsSweep(
                        stepCheckStartPos,
                        characterRotation,
                        -characterUp,
        ...
        private bool CheckStepValidity(int nbStepHits, Vector3 characterPosition, Quaternion characterRotation, Vector3 innerHitDirection, Vector3 stepCheckStartPos, out Collider hitCollider)
        {
            ...
            while (nbStepHits > 0 && !foundValidStepPosition)
            {
                ...
                {
                    int atStepOverlaps = StepCapsuleCollisionsOverlap(characterPositionAtHit, characterRotation, _internalProbedColliders);
                    if (atStepOverlaps <= 0)
                    {
                            ...
                            if (IsStableOnNormal(outerSlopeHit.normal))
                            {
                                        ...
                                        <= 0)
                                {
                                    ...
                                    if (innerStepValid)
                                    {
                                        ...
                                        _validStepColliders.Add(hitCollider);
                                        Debug.Log("1) Added. Count = " + _validStepColliders.Count);
                                        return true;
                                    }
                                }
                            }
        ...
        #region Overlaps and sweeps for character collider
        /// <summary>
        ...
        public int CharacterCollisionsOverlap(Vector3 atPosition, Quaternion atRotation, Collider[] overlappedColliders, float radiusInflate = 0f)
        {
            ...
            {
                if (!CheckIfColliderValidForCollisionsWithCharacter(overlappedColliders[i]))
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        public int CharacterOverlap(Vector3 atPosition, Quaternion atRotation, Collider[] overlappedColliders, LayerMask layers, QueryTriggerInteraction triggerInteraction, float radiusInflate = 0f)
        {
            ...
            // Filter out the character capsule and the StepCapsule
            nbHits = nbUnfilteredHits;
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                if (overlappedColliders[i] == Capsule || overlappedColliders[i] == StepCapsule)
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        public int CharacterCollisionsSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, float radiusInflate = 0f)
        {
            ...
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                ...
                if (hits[i].distance <= 0f ||
                    !CheckIfColliderValidForCollisionsWithCharacter(hits[i].collider))
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        public int CharacterSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, LayerMask layers, QueryTriggerInteraction triggerInteraction, float radiusInflate = 0f)
        {
            ...
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                // Filter out the character capsule and StepCapsule
                if (hits[i].distance <= 0f || hits[i].collider == Capsule || hits[i].collider == StepCapsule)
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        #region Overlaps and sweeps for Step collider
        /// <summary>
        /// Detect if the StepCapsule is overlapping with anything collidable
        /// </summary>
        /// <returns> Returns number of overlaps </returns>
        public int StepCapsuleCollisionsOverlap(Vector3 atPosition, Quaternion atRotation, Collider[] overlappedColliders, float radiusInflate = 0f)
        {
            int nbHits = 0;
            int nbUnfilteredHits = Physics.OverlapCapsuleNonAlloc(
                        atPosition + (atRotation * CharacterTransformToStepCapsuleBottomHemi),
                        atPosition + (atRotation * CharacterTransformToCapsuleTopHemi),
                        StepCapsule.radius + radiusInflate,
                        overlappedColliders,
                        CollidableLayers,
                        QueryTriggerInteraction.Ignore);
        ...
        /// <summary>
        /// Detect if the StepCapsule is overlapping with anything
        /// </summary>
        /// <returns> Returns number of overlaps </returns>
        public int StepCapsuleOverlap(Vector3 atPosition, Quaternion atRotation, Collider[] overlappedColliders, LayerMask layers, QueryTriggerInteraction triggerInteraction, float radiusInflate = 0f)
        {
            int nbHits = 0;
            int nbUnfilteredHits = Physics.OverlapCapsuleNonAlloc(
                        atPosition + (atRotation * CharacterTransformToStepCapsuleBottomHemi),
            ...
            // Filter out the Capsule and StepCapsule
            nbHits = nbUnfilteredHits;
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                if (overlappedColliders[i] == StepCapsule || overlappedColliders[i] == Capsule)
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        /// <summary>
        /// Sweeps the StepCapsule's volume to detect collision hits
        /// </summary>
        /// <returns> Returns the number of hits </returns>
        public int StepCapsuleCollisionsSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, float radiusInflate = 0f)
        {
            direction.Normalize();
            ...
            int nbHits = 0;
            int nbUnfilteredHits = Physics.CapsuleCastNonAlloc(
                    position + (rotation * CharacterTransformToStepCapsuleBottomHemi) - (direction * SweepProbingBackstepDistance),
                    position + (rotation * CharacterTransformToCapsuleTopHemi) - (direction * SweepProbingBackstepDistance),
                    StepCapsule.radius + radiusInflate,
                    direction,
                    hits,
        ...
        /// <summary>
        /// Sweeps the StepCapsule's volume to detect hits
        /// </summary>
        /// <returns> Returns the number of hits </returns>
        public int StepCapsuleSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, LayerMask layers, QueryTriggerInteraction triggerInteraction, float radiusInflate = 0f)
        {
            direction.Normalize();
            closestHit = new RaycastHit();
            ...
            int nbHits = 0;
            int nbUnfilteredHits = Physics.CapsuleCastNonAlloc(
                position + (rotation * CharacterTransformToStepCapsuleBottomHemi),
                position + (rotation * CharacterTransformToCapsuleTopHemi),
                StepCapsule.radius + radiusInflate,
                direction,
                hits,
            ...
            for (int i = nbUnfilteredHits - 1; i >= 0; i--)
            {
                // Filter out the StepCapsule
                if (hits[i].distance <= 0f || hits[i].collider == StepCapsule || hits[i].collider == Capsule)
                {
                    nbHits--;
                    if (i < nbHits)
                    {
        ...
        /// <summary>
        /// Casts the StepCapsule's volume in the character's downward direction to detect ground
        /// </summary>
        /// <returns> Returns the number of hits </returns>
        private bool CharacterGroundSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit)
        {
            ...
            // Capsule cast
            int nbUnfilteredHits = Physics.CapsuleCastNonAlloc(
                position + (rotation * CharacterTransformToStepCapsuleBottomHemi) - (direction * GroundProbingBackstepDistance),
            ...
            for (int i = 0; i < nbUnfilteredHits; i++)
            {
                ...
                if (_internalCharacterHits[i].distance > 0f && CheckIfColliderValidForCollisionsWithCharacter(_internalCharacterHits[i].collider))
                {
